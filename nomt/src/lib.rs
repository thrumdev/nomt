...
impl<T: HashAlgorithm> Nomt<T> {
    ...
    ///
    /// The [`Session`] is a kind of read transaction on the database. There may be multiple
    /// sessions live, though the existence of an outstanding session will prevent writes to the
    /// database. Sessions are the main way of reading to the database, and permit a changeset
    /// to be committed either directly to the database or into an in-memory [`Overlay`].
    pub fn begin_session(&self, params: SessionParams) -> Session {
        let live_overlay = params.overlay;

        let store = self.store.clone();
        let rollback_delta = if params.allow_rollback {
            self.store
                .rollback()
    ...
    pub fn rollback(&self, n: usize) -> anyhow::Result<()> {
        ...
        session_params.allow_rollback = false;
...
impl SessionParams {
    ...
    pub fn with_witness_mode(mut self, witness: WitnessMode) -> Self {
        self.witness = witness;
        self
    }

    /// Use a set of live overlays as a parent. Default: None
    pub fn with_overlay<'a>(
        mut self,
        overlays: impl IntoIterator<Item = &'a Overlay>,
    ) -> Result<Self, InvalidAncestors> {
        self.overlay = LiveOverlay::new(overlays)?;
        Ok(self)
    }
}
...
impl FinishedSession {
    ...
    ///
    /// If this session was configured with proving  (see [`SessionParams::with_witness_mode`]),
