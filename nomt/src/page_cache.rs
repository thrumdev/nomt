use crate::{
    ...
    rw_pass_cell::{ReadPass, Region, RegionContains, RwPassDomain, WritePass},
...
impl PageMut {
    ...
    pub fn from_data(data: FatPage) -> PageMut {
        PageMut {
...
impl CacheEntry {
    fn init(maybe_page: Option<(FatPage, BucketIndex)>) -> Self {
        match maybe_page {
            Some((data, bucket_index)) => CacheEntry {
                page_data: Arc::new(PageData::pristine_with_data(data)),
                bucket_index: Some(bucket_index),
            },
            None => CacheEntry {
                page_data: Arc::new(PageData::pristine_empty()),
                bucket_index: None,
            },
        }
    }
...
impl PageCache {
    ...
    pub fn new(
        ...
        Self {
            shared: Arc::new(Shared {
                ...
                root_page: RwLock::new(CacheEntry::init(root_page_data)),
    ...
    pub fn insert(&self, page_id: PageId, page: Option<(FatPage, BucketIndex)>) -> Page {
        ...
        let cache_entry = shard
            .cached
            .get_or_insert(page_id, || CacheEntry::init(page));
    ...
    pub fn prepare_transaction(
        ...
        for (page_id, page_diff) in page_diffs {
            ...
            if let Some(ref mut entry) = shard_guards[shard_index].cached.peek_mut(&page_id) {
                let page_data = &entry.page_data.data;
