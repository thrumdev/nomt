...
use crate::{
    ...
    merkle::page_walker::ElidedChildren,
...
use super::{
    ...
    page_walker::{PageSet as _, PAGE_ELISION_THRESHOLD},
...
impl SeekRequest {
    ...
    fn continue_leaves_fetch<H: HashAlgorithm>(
        ...
        if collected_leaf_data.is_empty() {
            let leaf_data = overlay_leaves.map(|(overlay_key, overlay_valuechange)| {
            ...
            final_leaf_data_collection.extend(leaf_data);
        } else {
            ...
            for (overlay_key, overlay_valuechange) in overlay.value_iter(range.0, range.1) {
                let key_path = collected_leaf_data[idx].0;
                let start_idx = idx;
                ...
                let value_hash = match overlay_valuechange {
                    ...
                    ValueChange::Delete if key_path == overlay_key => continue,
                    // PANIC: Deleted key must be present in key.
                    ValueChange::Delete => unreachable!(),
                };

                if key_path == overlay_key {
                    // The leaf data has been updated in the overlay.
                    idx += 1;
                }

                final_leaf_data_collection.push((key_path, value_hash));
            }
        }

        let pages = super::page_walker::reconstructor::reconstruct_pages::<H>(
            page,
            self.page_id.clone().unwrap(),
            self.position.clone(),
            page_set,
            std::mem::take(collected_leaf_data),
        );
...
impl<H: HashAlgorithm> Seeker<H> {
        ...
        while let Some(query) = request.next_query() {
            match query {
                IoQuery::MerklePage(page_id) => {
                    // Attempt to fetch a page from the page_set. If retrivial from memory
    ...
    fn handle_merkle_page_and_continue(
        ...
        page_set.insert(
            ...
            page.clone(),
            PageOrigin::HT(BucketInfo::Known(bucket_index)),
        );
