...
pub struct Shared {
    ...
    // This is used only during commit for now so probably a RwLock is useless
...
impl DB {
    pub fn open(num_rings: usize, path: PathBuf) -> anyhow::Result<Self> {
        ...
        // Spawn io_workers
        let (io_sender, io_receivers) = io::start_io_worker(
            N_WORKER,
            io::Mode::Real { num_rings }, // TODO: remove
        );
