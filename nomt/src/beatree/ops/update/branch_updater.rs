...
mod tests {
    ...
    fn update() {
        ...
        let DigestResult::Finished = updater.digest(&mut branch_changes) else {
            panic!()
        };

        let new_branch_entry = branch_changes.get(key(0)).unwrap();
    ...
    fn insert_overflowing() {
        ...
        let new_branch_entry_1 = branch_changes.get(key(0)).unwrap();
        let new_branch_1 = new_branch_entry_1.inserted.as_ref().unwrap();

        let new_branch_entry_2 = branch_changes.get(key(new_branch_1.n() as usize)).unwrap();
    ...
    fn delete_underflow_and_merge() {
        ...
        let DigestResult::NeedsMerge(_) = updater.digest(&mut branch_changes) else {
            panic!()
        };
    ...
    fn delete_completely() {
        ...
        assert!(branch_changes.get(key(0)).is_none());
    }
    ...
    fn delete_underflow_rightmost() {
        ...
        let new_branch_entry = branch_changes.get(key(499)).unwrap();
    ...
    fn shared_prefix_collapse() {
        ...
        let new_branch_entry_2 = branch_changes.get(key2(1)).unwrap();
        let new_branch_2 = new_branch_entry_2.inserted.as_ref().unwrap();
