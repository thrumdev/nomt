...
impl LeafUpdater {
    ...
    pub fn digest(&mut self, new_leaves: &mut impl HandleNewLeaf) -> DigestResult {
        ...
        // respecting the halful requirement will always be created.
        //
        // The only scenarion where the first leaf node creted will have a
        // body_size below the target is when there is an item who makes the
        // size to just from below to target to overful, but given the fact that
        // the maximum value size is `MAX_LEAF_VALUE_SIZE` the previoius schenarion
        // will only create nodes in the following range of body_size:
...
mod tests {
    use crate::beatree::{
        ...
        ops::update::leaf_updater::LeafGauge,
    };
    ...
    #[test]
    fn split_left_node_impossible_not_halfull() {
        let mut updater = LeafUpdater::new(PAGE_POOL.clone(), None, None);
        let mut new_leaves = TestHandleNewLeaf::default();
