...
use crate::{
    error::{InfallibleTaskResult, TaskResult},
...
pub struct SyncController {
    ...
    begin_sync_result_tx: Option<Sender<TaskResult<(), std::io::Error>>>,
    // The channel to receive the result of the begin sync task.
    begin_sync_result_rx: Receiver<TaskResult<(), std::io::Error>>,
...
struct SharedSyncController {
    ...
    pre_swap_rx: Mutex<Option<Receiver<InfallibleTaskResult<()>>>>,
}
...
impl SyncController {
    ...
    pub fn begin_sync(
        ...
        self.inner.sync.tp.execute(move || {
            ...
            task_result.with_context(format!("panic in beatree begin sync"));
    ...
    pub fn post_meta(&mut self) {
        ...
        // UNWRAPs: the offloaded non-critical sync work is infallible
        // and may fail only if it panics. It ill never produce an `std::io::Error`.
        let task_result = pre_swap_rx.recv().unwrap();
        task_result.handle().unwrap();
