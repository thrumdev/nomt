...
impl Meta {
    pub fn encode_to(&self, buf: &mut [u8; 20]) {
        ...
        buf[4..8].copy_from_slice(&self.ln_freelist_pn.to_le_bytes());
        buf[8..12].copy_from_slice(&self.ln_bump.to_le_bytes());
        buf[12..16].copy_from_slice(&self.bbn_freelist_pn.to_le_bytes());
        buf[16..20].copy_from_slice(&self.bbn_bump.to_le_bytes());
    }

    pub fn decode(buf: &[u8]) -> Self {
        let sync_seqn = u32::from_le_bytes(buf[..4].try_into().unwrap());
        let ln_freelist_pn = u32::from_le_bytes(buf[4..8].try_into().unwrap());
        let ln_bump = u32::from_le_bytes(buf[8..12].try_into().unwrap());
        let bbn_freelist_pn = u32::from_le_bytes(buf[12..16].try_into().unwrap());
