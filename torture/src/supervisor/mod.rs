...
async fn control_loop(
    ...
    // TODO: Reproducibility: in the vision of more complex resource allocation mechanisms
    // and swarm testing, requiring a single seed that can reproduce a workload
    // seems too big of a constraint. One way to enable reproducibility is to store all
    // the workload data needed to execute the workload in a file.
    //
    // NOTE: currently reproducibility is broken, will be fixed in a follow up.
    ...
    loop {
        ...
        loop {
            ...
            match resource_alloc.lock().unwrap().alloc(workload_id) {
                Ok(()) => (),
                Err(err) => {
