...
pub struct Workload {
    ...
    bitbox_seed: [u8; 16],
    /// Data collected to evaluate the average commit time in milliseconds.
    tot_commit_time: u64,
    ...
    scheduled_rollback: Option<(ScheduledRollback, Option<u64>)>,
}
...
impl Workload {
    ...
    async fn exercise_commit_crashing(
        ...
        let mut crash_delay = self
            ...
            .unwrap_or(Duration::from_millis(300).as_millis() as u64);
    ...
    async fn schedule_rollback(&mut self, should_crash: bool) -> anyhow::Result<()> {
        ...
        let maybe_crash_delay = if should_crash {
            ...
            Some(Duration::from_millis(10).as_millis() as u64)
        } else {
            None
        };
