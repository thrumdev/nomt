...
impl Workload {
    pub fn new(
        ...
        let config = WorklaodConfiguration::new(&mut rng, resource_alloc.clone(), workload_id)?;
    ...
    async fn ensure_snapshot_validity(&mut self) -> anyhow::Result<()> {
        if !self.config.is_ensure_snapshot() && !self.config.sample_snapshot {
            return Ok(());
        }
        ...
        if self.config.is_ensure_snapshot() {
    ...
    fn gen_key_value_change(
        ...
        if !self.committed.state.is_empty() || used_keys.len() == self.committed.state.len() {
            if self.config.new_key == 0.0 {
                return None;
            } else {
        ...
        match change_type {
            0 => {
                ...
                return Some(KeyValueChange::Delete(key));
            }
            1 => {
                let key = find_existing_key(&mut self.rng);
                used_keys.insert(key.clone());
