...
use super::{
    cli::WorkloadParams,
    resource::{ResourceAllocator, ResourceExhaustation},
};

/// Percentage of the assigned size to the workload that will be
/// used by the hash table.
const HASH_TABLE_SIZE_RATIO: f64 = 0.8;

/// Maximum size of a single commit, combining reads and updates.
const MAX_COMMIT_SIZE: usize = 1000;

/// Maximum number of times a workload is repeated
const MAX_ITERATIONS: usize = 10;

/// Maximum numbre of commit workers supported by nomt.
const MAX_COMMIT_CONCURRENCY: usize = 64;

/// Maximum numbre of io workers supported by nomt.
...
/// Max size in bytes supported by trickfs.
///
/// Note that TRICKFS works entirely in memory,
/// thus this threshold refers to memory usage and not disk usage.
const TRICKFS_MEMORY_THRESHOLD: u64 = 5 * (1 << 30); // 5GiB

#[derive(Debug)]
pub struct WorklaodConfiguration {
...
impl WorklaodConfiguration {
    ...
    pub fn is_ensure_snapshot(&mut self) -> bool {
