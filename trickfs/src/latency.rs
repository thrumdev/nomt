...
fn schedule_new_reply(scheduled: &mut VecDeque<(Reply, Instant)>, reply: Reply, delay: Duration) {
    let new_deadline = std::time::Instant::now() + delay.clone();
    // If two replies happen to have the same deadline, then they will be kept in FIFO order.
    let idx = match scheduled.binary_search_by_key(&new_deadline, |(_, deadline)| *deadline) {
        Ok(idx) => idx + 1,
        Err(idx) => idx,
    };
    scheduled.insert(idx, (reply, new_deadline));
}
