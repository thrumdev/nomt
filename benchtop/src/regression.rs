...
pub fn regression(params: Params) -> Result<()> {
    let mut regr_options: RegressionInputs = match &params.input_file {
        Some(input_path) => {
            // load toml file
    ...
    for (workload_name, workload_info) in regr_options.workloads.iter_mut() {
        // for each specified workload, execute it and compare the result if specified
    ...
    if let Some(output_path) = params.output_file {
        std::fs::write(output_path, toml::to_string(&regr_options)?)?;
    }
